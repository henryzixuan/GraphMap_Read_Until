...
RegionSelectionBins::RegionSelectionBins(std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes) {
  int64_t bin_size = /*(parameters->overlapper == true) ? -1 :*/ 250 / 3;
  float bin_size_inverse = (bin_size > 0) ? (1.0f / ((float) bin_size)) : (0.0f);

  int64_t num_ref_chrom = 455; //
  // std::cout<<"RegionSelectionBins Constructor called, space allocated for "<<indexes[0]->get_num_sequences_forward()<<" sets of bins"<<std::endl;
 
  bins_chromosome.resize(num_ref_chrom * 2);
  last_update_chromosome.resize(num_ref_chrom* 2);
  for (int64_t i = 0; i < num_ref_chrom; i++) {
    ...
    bins_chromosome[i + num_ref_chrom].resize(current_num_bins, 0);
    last_update_chromosome[i + num_ref_chrom].resize(current_num_bins, 0);

    // std::cout<<"bins_chromosome["<<i<<"].size(): "<<bins_chromosome[i].size()<<std::endl; 
  }
}

void RegionSelectionBins::Clear(){
  for (int64_t i = 0; i < /*indexes[0]->get_num_sequences_forward()*/ 455 * 2; i++) {
    std::fill(bins_chromosome[i].begin(),bins_chromosome[i].end(),0.0f);
    std::fill(last_update_chromosome[i].begin(),last_update_chromosome[i].end(),0);
  }
}

void RegionSelectionBins::Print(){
  int64_t zero_count;
  for (int64_t i = 0; i < /*indexes[0]->get_num_sequences_forward()*/ 455 * 2; i++) {
    zero_count = 0;
    for(int64_t j = 0; j < bins_chromosome[i].size(); j++){
      if(bins_chromosome[i][j] < 0.001f){
        zero_count+=1;
      }
    }
    std::cout<<"bins_chromosome["<<i<<"]: "<<bins_chromosome[i].size()<<" bins, "<<((float)zero_count/(float)bins_chromosome[i].size())*100<< "% zeros" << std::endl; 
  }
}
