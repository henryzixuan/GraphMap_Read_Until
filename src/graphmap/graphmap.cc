...
//  if (parameters->num_threads > 0)
  ...
  std::cout<<"Allocating RegionSelectionBins for each thread:"<<std::endl;
  std::vector<RegionSelectionBins *> region_selection_bins_vec;
  region_selection_bins_vec.resize(num_threads);
  for(int64_t t = 0; t < num_threads; t++){
    region_selection_bins_vec[t] = new RegionSelectionBins(indexes_);
    std::cout<<t<<" ";
  }
  std::cout<<std::endl<<"Done Allocating RegionSelectionBins"<<std::endl;
...
//        ss << "\n";
    ...
    mapping_data->region_selection_bins = region_selection_bins_vec[thread_id];
    // std::cout<<"THREAD ID: "<<thread_id<<", bins_chromosome.size(): "<<mapping_data->region_selection_bins->bins_chromosome.size()<<std::endl;
...
//    mapping_data.reset();
  ...
  //delete 
  std::cout<<"Deleting RegionSelectionBins for each thread"<<std::endl;
  for(int64_t t = 0; t < num_threads; t++){
    delete region_selection_bins_vec[t];
  }
  std::cout<<"Done Deleting RegionSelectionBins"<<std::endl;
