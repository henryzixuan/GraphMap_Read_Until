...
int GraphMap::RegionSelectionNoCopy_(int64_t bin_size, MappingData* mapping_data, std::vector<std::shared_ptr<is::MinimizerIndex>> &indexes, const SingleSequence* read, const ProgramParameters* parameters) {
  ...
  // mapping_data->region_selection_bins->Clear();
...
//            fflush(stdout);
            ...
            if (mapping_data->region_selection_bins->last_update_chromosome[reference_index][position_bin] == (i + 1)) {
              continue;
            }
            if (reference_index >= mapping_data->region_selection_bins->bins_chromosome.size() ||
                position_bin >= mapping_data->region_selection_bins->bins_chromosome[reference_index].size()/*TODO: CAHNGE*/) {
              continue;
            }

            mapping_data->region_selection_bins->bins_chromosome[reference_index][position_bin] += 1.0f;
            if (mapping_data->region_selection_bins->bins_chromosome[reference_index][position_bin] > max_bin_value) { max_bin_value = mapping_data->region_selection_bins->bins_chromosome[reference_index][position_bin]; }

            mapping_data->region_selection_bins->last_update_chromosome[reference_index][position_bin] = (i + 1);
...
//  LOG_DEBUG_HIGH("total_num_hits = %ld\n", total_num_hits);
  ...
  for (int64_t i = 0; i < (indexes[0]->get_num_sequences_forward() * 2); i++) {
    for (int64_t j = 0; j < mapping_data->region_selection_bins->bins_chromosome[i].size(); j++) {
      if (mapping_data->region_selection_bins->bins_chromosome[i][j] > min_allowed_bin_value) { num_bins_above_min += 1; }
    }
  }
  ...
  for (int64_t i = 0; i < (indexes[0]->get_num_sequences_forward() * 2); i++) {
    for (int64_t j = 0; j < mapping_data->region_selection_bins->bins_chromosome[i].size(); j++) {
      if (mapping_data->region_selection_bins->bins_chromosome[i][j] > min_allowed_bin_value) {
        ChromosomeBin new_bin;
        new_bin.reference_id = i;
        new_bin.bin_id = j;
        new_bin.bin_value = mapping_data->region_selection_bins->bins_chromosome[i][j];
        if (j > 0) { new_bin.bin_value += mapping_data->region_selection_bins->bins_chromosome[i][j-1] / 2.0f; }
        if ((j + 1) < mapping_data->region_selection_bins->bins_chromosome[i].size()) { new_bin.bin_value += mapping_data->region_selection_bins->bins_chromosome[i][j+1] / 2.0f; }
        mapping_data->bins.push_back(new_bin);
      }
      mapping_data->region_selection_bins->bins_chromosome[i][j] = 0.0f; //clear for next iteration
      mapping_data->region_selection_bins->last_update_chromosome[i][j] = 0.0f; //clear for next iteration
    }
  }
...
#ifdef PRINT_RS
  printf("Region selection timings:\n");
  printf("    time_region_seed_lookup = %f\n", mapping_data->time_region_seed_lookup);
  printf("    time_region_alloc = %f\n", mapping_data->time_region_alloc);
  printf("    time_region_counting = %f\n", mapping_data->time_region_counting);
  printf("    time_region_conversion = %f\n", mapping_data->time_region_conversion);
  printf("    time_region_sort = %f\n", mapping_data->time_region_hitsort);
  printf("\n");
  printf("    total_num_hits = %ld\n", total_num_hits);
  printf("    read_len = %ld\n", read->get_sequence_length());
#endif

// mapping_data->region_selection_bins->Print();
//  exit(1);
